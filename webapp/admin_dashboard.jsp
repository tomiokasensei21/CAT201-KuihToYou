...
<head>
    ...
    <style>
        ...
        .header-section h1 { font-family: 'Playfair Display', serif; font-size: 42px; margin: 0; }

        .btn-add {
            background: var(--clay-orange); color: white; border: none;
            padding: 14px 30px; border-radius: 50px; cursor: pointer;
            font-weight: bold; text-transform: uppercase; letter-spacing: 1px;
            box-shadow: 0 5px 15px rgba(185, 124, 90, 0.3); transition: 0.3s;
        }
        .btn-add:hover { background: var(--clay-dark); transform: translateY(-2px); }

        .table-card {
            background: var(--warm-white); border-radius: 20px;
            padding: 30px; box-shadow: 0 10px 30px var(--warm-shadow);
        }

        .search-bar {
            width: 100%; padding: 15px 20px; border-radius: 50px;
            border: 1px solid rgba(185, 124, 90, 0.2); background: #fdfaf5;
            font-family: 'Lora'; font-size: 15px; margin-bottom: 25px;
        ...
        th {
            ...
            text-transform: uppercase; font-size: 11px; letter-spacing: 1.5px; color: #999;
        }
        td { padding: 15px; border-bottom: 1px solid var(--parchment-bg); }

        .img-preview { width: 55px; height: 55px; border-radius: 12px; object-fit: cover; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }

        .stock-badge { padding: 5px 12px; border-radius: 50px; font-size: 12px; font-weight: bold; display: inline-block; }
...
<div class="admin-container">
    <div class="header-section">
        ...
        <button class="btn-add" onclick="openKuihModal()">+ Add New Kuih</button>
    </div>

    <div class="table-card">
        <input type="text" id="searchInput" class="search-bar" placeholder="Filter by kuih name..." onkeyup="filterTable()">
        <table>
            <thead>
            <tr>
                <th>Preview</th>
...
<script>
    ...
    async function loadData() {
        try {
            ...
            kuihList = data.products || [];
            availableImages = data.images || [];
            renderTable(kuihList);
    ...
    async function openKuihModal(id = null) {
        ...
        const { value: formValues } = await Swal.fire({
            ...
            confirmButtonText: isEdit ? 'Save Changes' : 'Add to Inventory',
    ...
    async function deleteKuih(id) {
        ...
        if (result.isConfirmed) send('delete', { id });
    }

    async function send(action, data) {
        const p = new URLSearchParams({ action, ...data });
        await fetch('admin-api', { method: 'POST', body: p });
        loadData();
    ...
    function filterTable() {
        ...
        renderTable(kuihList.filter(k => k.name.toLowerCase().includes(term)));
    }
