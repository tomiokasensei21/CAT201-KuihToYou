    ...
    public class DataHandler {
        ...
        // ADD THIS FOR MEMBER 3: SAVE NEW USER
        public static void saveUser(String username, String password, ServletContext context) {
            String filePath = context.getRealPath("/data/users.txt");
            // 'true' in FileWriter means "Append" - it adds to the bottom of the file
        ...
        // ADD THIS FOR MEMBER 3: SAVE ORDER
        public static void saveOrder(String orderDetails, ServletContext context) {
            String filePath = context.getRealPath("/data/orders.txt");
            try (PrintWriter out = new PrintWriter(new FileWriter(filePath, true))) {
                out.println("Order Date: " + new Date() + " | " + orderDetails);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
